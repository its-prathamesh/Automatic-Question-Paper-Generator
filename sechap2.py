...
def sechap2():
    a=connect("pvp.db")
    d=a.cursor()
    d.execute("select *from sechap2 where Q_no>1 and Q_no<=25")
    data=d.fetchall()
    ...
    def insert():
            a=connect("pvp.db")
            b=a.cursor()
            b.execute("create table if not exists sechap2(Q_no int ,Question text)")
            var = entry.get('1.0','end-1c')
            var1=entry1.get('1.0','end-1c')
            var2=entry2.get('1.0','end-1c')
            b.execute("insert into sechap2 (Q_no,Question,Type,Marks) values(?,?,?,?)",(qno.get(),var,var1,var2))
    ...
    def update():
            ...
            b=a.cursor()
            var = entry.get('1.0','end-1c')
            b.execute("update sechap2 set Question = (?) where Q_no = (?) ",(var,qno.get(),))
            entry.delete('1.0',END)
            a.commit()
            qno.delete(0, END)
            updatetype()
    ...
    def updatetype():
            ...
            b=a.cursor()
            var1= entry1.get('1.0','end-1c')
            b.execute("update sechap2 set Type = (?) where Q_no = (?) ",(var1,qno.get(),))
            entry1.delete('1.0',END)
            a.commit()
            qno.delete(0, END)
            updatemarks()
    ...
    def updatemarks():
            ...
            b=a.cursor()
            var2 = entry2.get('1.0','end-1c')
            b.execute("update sechap2 set Marks = (?) where Q_no = (?) ",(var2,qno.get(),))
            entry2.delete('1.0',END)
            qno.delete(0, END)
            a.commit()
    ...
    def ext():
            a=connect("pvp.db")
            b=a.cursor()
            b.execute("select Question from sechap2 where Q_no = (?)",(qno.get(),))
            result = b.fetchall()
            for i in result:
    ...
    def exttype():
            a=connect("pvp.db")
            b=a.cursor()
            b.execute("select Type from sechap2 where Q_no = (?)",(qno.get(),))
            result = b.fetchall()
            for i in result:
                    for j in range(len(i)):
    ...
    def extmark():
            ...
            a=connect("pvp.db")
            b=a.cursor()
            b.execute("select Marks from sechap2 where Q_no = (?)",(qno.get(),))
            result = b.fetchall()
            for i in result:
                    for j in range(len(i)):
    ...
    def delete():
            ...
            var1=entry1.get('1.0','end-1c')
            var2=entry2.get('1.0','end-1c')
            b.execute("delete from sechap2 where Question = (?) and Q_no = (?) ",(var,qno.get(),))
            entry.delete('1.0',END)
            entry1.delete('1.0',END)
            entry2.delete('1.0',END)
            qno.delete(0, END)
...
def next():
    ...
    a=connect("pvp.db")
    d=a.cursor()
    d.execute("select *from sechap2 where Q_no>25 and Q_no<=50")
    data=d.fetchall()
